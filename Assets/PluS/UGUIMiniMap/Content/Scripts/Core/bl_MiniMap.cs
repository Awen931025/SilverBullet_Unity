...
public class bl_MiniMap : MonoBehaviour
{
    ...
    public Sprite littleOutlineMask;
    public Sprite bigOutlineMask;
    ...
    [Header("固定离目标点的gaodu ")]
    public bool isConstantHeight = true;
    public float constArea = 10;
    public float constheight = 10;
    ...
    public static RectTransform MaskUI { get { return MapUIRoot.GetSon("Mask").GetComponent<RectTransform>(); } }
    ...
    void PositionControll()
    {
        ...
        if (isConstantHeight)
        {
            p.y = (constheight + constheight / 2) + (Target.position.y * 2);
        }
        else
        {
    ...
    void RotationControll()
    {
        ...
        {
            float yAxis = m_Target.transform.localEulerAngles.y;
            PlayerIcon.transform.parent.localEulerAngles = new Vector3(0, 0, 360 - yAxis);
    ...
    void MapSize()
    {
        ...
        if (isConstantHeight)
        {
            MMCamera.orthographicSize = constArea;
        }
        else
        {
    ...
    private bl_MaskHelper m_maskHelper
    {
        get
        {
            if (_maskHelper == null)
            {
                _maskHelper = m_Canvas.GetComponentInChildren<bl_MaskHelper>();
            }
            return _maskHelper;
        }
    }
    ...
    public void ShowBig()
    {
        ...
        useCompassRotation = false;
        PlayerIcon.transform.parent.localEulerAngles = new Vector3(0, 0, 0);
        height = PlayerPrefs.GetFloat(MMHeightKey, height);
        if (useCompassRotation != DefaultRotationCircle) { useCompassRotation = DefaultRotationCircle; }
    ...
    public void ShowCamreaAndCanvas()
    {
