...
public class ExcelTable
{
    ...
    public bool GetIsMerged(int row, int column)
    {
        string mergedStr = excelWorksheet.MergedCells[row, column];
        if (mergedStr!=null)
        {
            return true;
        }
    ...
    /// <summary>
    /// mfy-得到合并单元格的值,如果不是合并单元格则返回普通行的值
    /// </summary>
    /// <param name="wSheet"></param>
    /// <param name="row"></param>
    /// <param name="column"></param>
    /// <returns></returns>
    public string GetMegerValue(int row, int column)
    {
        ...
        if (range == null)
        {
            if (excelWorksheet.Cells[row, column].Value != null)
                return excelWorksheet.Cells[row, column].Value.ToString();
            else
                return "";
        }
        object value = excelWorksheet.Cells[(new ExcelAddress(range)).Start.Row, (new ExcelAddress(range)).Start.Column].Value;
        if (value != null)
