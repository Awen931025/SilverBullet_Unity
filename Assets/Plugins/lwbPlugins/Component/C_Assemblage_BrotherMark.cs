...
等于总数是分散")]
        ...
        void Log_StepName_Complete_Init()
        {
            if (!logStepOnComplete)
                return;
            Chai_Step_Complete.AddListener(Chai_Step_Function);
            Zhuang_Step_Complete.AddListener(Zhuang_Step_Function);
        }
        void Log_GroupIndex_Complete_Init()
        {
            if (!logGroupOnComplete)
                return;
            Chai_Group_Complete.AddListener(Chai_Group_Function);
            Zhuang_Group_Complete.AddListener(Zhuang_Group_Function);
        }
        //哪个物体，什么事件
        ...
        public void OriGroupEffect(int index)
        {
            ...
            foreach (Transform tran in tranS)
            {
                tran.GetComponent<OutlineObj>().FlashingOn();
            }
        }
    }
}