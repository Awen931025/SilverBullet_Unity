...
namespace W
{
    public class C_Camera_BaseDisplay : MonoBehaviour
    {
        ...
        [Header("所在UI（只有在这个区域点击才有拖动效果，空值则始终有效）")]
        ...
        {
            if (Input.GetMouseButtonDown(leftKeyOrRightKey))
            {
                //if (EventSystem.current.IsPointerOverGameObject())
                //{
                //    if (targetUI)
                //    {
                //        canMove = RectTransformUtility.RectangleContainsScreenPoint(targetUI, Input.mousePosition);
                //    }
                //    else
                //    {
                //        canMove = true;
        ...
        void HandlePlayerInput()
        {
            ...
            if (targetUI)
            {
                if (EventSystem.current.IsPointerOverGameObject())
                {
                    canWheelScale = RectTransformUtility.RectangleContainsScreenPoint(targetUI, Input.mousePosition);
                }
                else
                {
                    canWheelScale = false;
